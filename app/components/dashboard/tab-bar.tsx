...
const TabBar = () => {
    return (
        <TabsList className='gap-2.5 px-1 bg-white py-6 rounded-r-full rounded-l-full'>
            {
                tabItems.map((item, index) => <TabsTrigger className='py-5 text-sm px-3 duration-300 rounded-full hover:scale-105 hover:bg-green/90 ' key={index} value={item.key}>{item.value}</TabsTrigger>)
            }
        </TabsList>
