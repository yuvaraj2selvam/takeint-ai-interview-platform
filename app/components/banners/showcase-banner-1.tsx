    ...
    return (<>
        <article className='pt-10 pb-10'>
            <section className='flex flex-row justify-items-start gap-10 pt-12 pb-12 items-center'>
                <h3 className='w-fit green-head text-center sm:text-left px-4 text-2xl sm:text-4xl'>Unlock Your Interview Success</h3>
                <p className='max-w-2/3'>Transform your interview skills with AI-driven mock interviews that mimic
            ...
            <section className='grid md:grid-cols-2 grid-cols-1 gap-10 items-center'>
                {
                    containerItems.map((item, index) => {
                        return <div key={index} className={`bg-${item.bg} h-[260px] w-full  flex flex-col gap-8 justify-items-start justify-around border-[1px] border-black border-b-8 p-10 rounded-4xl`}>
